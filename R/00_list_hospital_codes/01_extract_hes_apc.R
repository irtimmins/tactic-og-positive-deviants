# =============================================================================
# 01  Cut the HES-APC extract down to the cohort
# -----------------------------------------------------------------------------
# The raw HES-APC file covers far more patients and columns than this project
# needs, and reading it takes several minutes. This makes the smaller, faster
# copy every later HES read reuses (f_hes_extract): the same episodes, cut to
# the patients in the rapid extract and to the columns the endoscopy match uses.
#
# It sits in this stage, rather than in the endoscopy stage that consumes it,
# because 02_list_site_codes.R needs the HES site field too. Doing both here
# means the large file is read once, not twice, and the endoscopy stage can
# assume the extract already exists.
#
...
# denominator downstream is the real one.
#
...
source("R/config/directories.R")   # path_rapid_dta, path_hes_apc_dta, f_hes_extract
...
cat("01 complete. Next: 02_list_site_codes.R\n")