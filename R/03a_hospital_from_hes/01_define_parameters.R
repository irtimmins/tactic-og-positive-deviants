...
source("R/03a_hospital_from_hes/_load_packages.R")
source("R/03a_hospital_from_hes/_helpers.R")
...
# sourced above, so stage 05 can read f_endoscopy_lookup without sourcing this
# file to find out what it is called. The cohort-filtered HES extract
# (f_hes_extract) is written by stage 00; path_hes_apc_dta is only the fallback
# for when that has not been run.
