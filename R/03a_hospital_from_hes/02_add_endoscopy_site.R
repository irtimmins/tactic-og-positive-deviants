...
#
# Three flags are carried alongside, so every anchored patient falls into
# exactly one outcome and the misses can be told apart:
#   endo_in_hes   the patient appears in the HES-APC extract at all
#   endo_has_apc  they have an APC diagnostic endoscopy code, on any date
#   endo_matched  that endoscopy sat within the window of the recorded date
# A site can still be missing after a match, when the episode's sitetret is
# blank or a default - which is a missing SITE, not a missing endoscopy. Keeping
# endo_matched separate is what stops the two being reported as the same thing,
# and together they answer "do we need HES-OP as well".
#
# Reads : the rapid tumour dta, and the cohort-filtered HES-APC extract that
#         stage 00 writes (falling back to the raw APC dta if it is absent)
