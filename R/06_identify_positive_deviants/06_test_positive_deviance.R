...
#   Rscript R/tests/06_test_positive_deviance.R
# =============================================================================
...
# this test spans two stages: the cohort funnel it exercises now lives in stage
# 05, and the modelling in stage 06. Sourcing stage 06's config is enough for
# both, since that file sources stage 05's config first.
r_dir_05 <- "R/05_derive_analysis_cohort"
r_dir_06 <- "R/06_identify_positive_deviants"
...
suppressWarnings(suppressMessages(source(file.path(r_dir_06, "01_config.R"))))
...
mk <- function(n, site, endo = TRUE, stage = 1L, path = "Surgery only",
               wait = 30L) tibble(
                 ...
                 # both hospital fields carry the same code here, so this fixture
                 # exercises the funnel logic the same way whichever one hosp_var
                 # (in 01_config.R) is currently pointed at - the two fields
                 # legitimately differ on real data, but that difference is not
                 # what this test is about.
                 site_dx_code = site, endoscopy_site = site,
                 tumour_site = "C15", tx_pathway = path,
                 ...
                 imd_2019_RR = 3L, diagnosis_alliance = "Alliance A",
                 # an elective route, so these fixture patients survive the
                 # route-to-diagnosis exclusion in 02 and the counts this test
                 # asserts stay driven by the criteria it is actually checking
                 tfinal_route = "TWW",
                 sotn_cohort = 1L)
...
# run 02 against it. 02 sources stage 05's 01_config, which resets in_rds to
...
invisible(capture.output(suppressMessages(
  sys.source(file.path(r_dir_05, "02_build_cohort.R"), flow_env))))
...
if (requireNamespace("rstan", quietly = TRUE) && file.exists(stan_file)) {
  ...
  #
  # Compiling the Stan model is wrapped, because rstan being INSTALLED is not
  # the same as rstan being able to BUILD: a machine can have the package but
  # not the C++ toolchain it needs (missing Boost headers, no compiler). That
  # is a missing environment, not a logic failure, so it is reported as a skip
  # the same way an absent package is - otherwise the whole test file is marked
  # failed for a reason that has nothing to do with the code being checked.
  y  <- c(30, 40, 50, 90)          # site 4 is an outlier
  se <- c(2, 2, 2, 20)             # and imprecise
  fit <- tryCatch(suppressWarnings(fit_shrink(y, se, refresh = 0)),
                  error = function(e) e)
  if (inherits(fit, "error")) {
    cat("  (skipped - rstan cannot build the model here: ",
