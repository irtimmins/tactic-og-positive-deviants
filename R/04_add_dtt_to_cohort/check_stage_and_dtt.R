# =============================================================================
# check  Stage, pathway and the non-positive decision-to-treat intervals
# -----------------------------------------------------------------------------
# A one-off look, not part of the build: it reads what 03 wrote, prints, and
# writes nothing. The runner does not call it - run it by hand when the question
# comes up.
#
# The question it was written for: 03 marks a decision-to-treat interval
# "non-positive" when the treatment date is not after the decision date, and
# that reason was firing more often than the mature cohort led us to expect. The
# suspicion was stage 4 - where treatment is often palliative and can start
# effectively at the decision - rather than a fault in the merge.
#
# So it splits tx_pathway and dtt_to_tx_reason by stage, then asks the question
# directly: does the non-positive rate fall away once stage 4 and unstaged
# patients are set aside? If it does, the reason is clinical and the merge is
# behaving; if it does not, the merge needs another look.
#
# Reads : og_cohort_cwt.rds (whatever 03 last wrote)
# Writes: nothing
# =============================================================================
