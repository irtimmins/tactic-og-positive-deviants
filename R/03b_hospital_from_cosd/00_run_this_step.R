...
# dir_out <- "Output/sim/patient_level" for a trial run against synthetic data, or
...
# more. Wrapping the run in run_this_step() gives on.exit something real to hang
...
run_this_step <- function() {
  dir_build <- "R/03b_hospital_from_cosd"
  step <- function(file) {
    cat("\n========== ", file, " ==========\n", sep = "")
  ...
  if (!is.null(dir_transfer)) {
    ...
    log_file <- file.path(dir_transfer,
                          paste0("cosd_site_log_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".txt"))
  ...
  # the logic checks for this stage live in R/tests/03b_test_hospital_from_cosd.R
  #
  # check_site_ambiguity_after_trust_match.R sits in this folder but is not run
  # here: it answers a one-off question about how much COSD site ambiguity
  # survives trust-matching, and changes nothing the build produces. Run it by
  # hand when the question comes up again.
