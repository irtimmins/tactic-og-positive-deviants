...
#   hospital funnel  the endoscopy hospital (from HES) is the unit; a site is
#                    kept only if it has at least min_per_year diagnoses in
#                    every year of the window. Excluded sites are listed with
#                    their yearly counts. Set hosp_var in 01_config.R to use the
#                    COSD site of diagnosis instead.
#
...
# the endoscopy hospital, from the HES stage ---------------------------------
# 03a_hospital_from_hes writes a patient -> five-character site lookup
# for the hospital that did the diagnostic endoscopy. That is a different unit
# from the COSD site of diagnosis (site_dx_code): it is where the clock actually
# started, so it is the more natural hospital for an endoscopy-to-DTT outcome.
# Join it in here and set hosp_var in 01_config.R to choose between them.
#
# The join is optional so this stage still runs where the endoscopy build has not
# been run - the column simply arrives empty, and a hosp_var pointing at it would
# then drop everyone at the "site of diagnosis recorded" step rather than fail
# obscurely.
f_endo_lookup <- get0("f_endoscopy_lookup",
                      ifnotfound = file.path(dirname(in_rds),
                                             "og_endoscopy_hes_lookup.rds"))
if (file.exists(f_endo_lookup)) {
  endo_site <- readRDS(f_endo_lookup) %>%
    select(patient_pseudo_id, endoscopy_site, endoscopy_provider)
  og <- og %>%
    mutate(patient_pseudo_id = as.character(patient_pseudo_id)) %>%
    left_join(endo_site, by = "patient_pseudo_id")
  cat(sprintf("joined an endoscopy hospital for %d of %d patients (%.1f%%)\n",
              sum(!is.na(og$endoscopy_site)), nrow(og),
              100 * mean(!is.na(og$endoscopy_site))))
} else {
  cat("note: no endoscopy-site lookup at", f_endo_lookup,
      "- endoscopy_site left empty\n")
  # only fills the column in if it is not already there, so this never clobbers
  # an endoscopy_site a caller has already attached (as the test fixtures do) -
  # on real data og_cohort_cwt.rds never carries this column itself, so the
  # branch behaves exactly as before there.
  if (!"endoscopy_site" %in% names(og))
    og <- og %>% mutate(endoscopy_site = NA_character_,
                        endoscopy_provider = NA_character_)
}
...
# is dropped, then keep only the curative-inclusion pathways. The guard means an
# empty pathways_flagged skips the step rather than logging a no-op exclusion.
n_flagged <- 0L
...
# route to diagnosis ---------------------------------------------------------
# keep the elective routes only; emergency presentations and unrecorded routes
# are excluded (see routes_exclude in the config for why). The breakdown of what
# is being dropped is printed rather than just counted, so an unexpected value in
# the field is visible here rather than silently removed.
if ("tfinal_route" %in% names(df)) {
  dropped_route <- df %>%
    filter(tfinal_route %in% routes_exclude) %>%
    count(tfinal_route, name = "patients") %>%
    arrange(desc(patients))
  if (nrow(dropped_route)) {
    cat("\nExcluded by route to diagnosis:\n")
    print(as.data.frame(dropped_route), row.names = FALSE)
  }
  df <- df %>% filter(!tfinal_route %in% routes_exclude)
  fc(sprintf("Elective route to diagnosis (excluding %s)",
             paste(routes_exclude, collapse = ", ")), df)
} else {
  cat("note: no tfinal_route column - route-to-diagnosis exclusion skipped\n")
}
...
fc(sprintf("Hospital recorded (%s)", hosp_var), df,
...
if (file.exists(site_trust_map_csv)) {
  ...
  # separate from site_map so df's own columns are unaffected here. ODS returns
  # organisation names in capitals, so title_case tidies it here, once, rather
  # than every place the name is later displayed.
  hosp_name_lookup <- read.csv(site_trust_map_csv, colClasses = "character") %>%
    distinct(site_code, name) %>%
    mutate(name = title_case(name))
  df <- df %>%
