...
# Writes: Data/OG/og_site_of_diagnosis_lookup.rds  (one row per patient)
...
# the SNOMED map fetched by R/reference/10_fetch_snomed_map.R, if it is there
...
if (!is.null(from_map)) {
  ...
  cat("No SNOMED map at", f_snomed_map,
      "- run R/reference/10_fetch_snomed_map.R on a networked machine and copy",
      "the result over.\n")
}
